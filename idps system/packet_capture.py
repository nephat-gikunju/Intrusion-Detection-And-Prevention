        ...
        if IP in packet:
            ...
            if proto == 6:  # TCP
                if TCP in packet:
                    ...
                    log_msg = f"TCP Packet: {ip_src}:{tcp_sport} -> {ip_dst}:{tcp_dport}"
                    print(log_msg)
                ...
                if UDP in packet:
                    ...
                    log_msg = f"UDP Packet: {ip_src}:{udp_sport} -> {ip_dst}:{udp_dport}"
                    print(log_msg)
                    logging.info(log_msg)
            
            else:
                log_msg = f"Other IP Packet: {ip_src} -> {ip_dst} (Protocol: {proto})"
                print(log_msg)
                logging.info(log_msg)
